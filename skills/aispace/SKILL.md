---
name: aispace
description: Upload, securely share, receive, and manage temporary files with the aispace CLI. Use when an agent needs to hand off an artifact through an expiring link, use optional client-side age encryption, decrypt a received aispace file locally, inspect quota, or revoke/delete a prior share.
---
...
```sh
producer | aispace upload - --name result.json --link --link-expires 1h --json
```
...
the storage operator should not see:

```sh
aispace upload PATH --encrypt --identity-out PATH.agekey --link --link-expires 1h --max-downloads 1 --json
```
...
- Deliver the share URL and identity through separate authenticated channels when practical.
...
```sh
aispace upload PATH --encrypt --recipient 'age1...' --link --json
```
...
- List live files: `aispace ls --json`
...
- Delete a file and invalidate all its links: `aispace rm FILE_ID`
...
Return the share URL, effective expiration, download cap, whether client encryption was used, and
the local identity-file path when one was created. Never reproduce the identity itself unless the
user explicitly asks to transfer that secret in the current channel.
